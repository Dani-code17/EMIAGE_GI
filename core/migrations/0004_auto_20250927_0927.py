...
def attach_documents_to_ecue(apps, schema_editor):
    ...
    print('[migration 0004] Rattachement des documents aux ECUE...')
    ...
    for code, name, level, semester, ecues in ues_data:
        ue, _ = UE.objects.get_or_create(
            ...
            defaults={'slug': slugify(f'{level}-{semester}-{code}-{name}')},
        )
        for ecue_name in ecues:
            ECUE.objects.get_or_create(
                name=ecue_name, ue=ue,
                defaults={'slug': slugify(f'{ue.level}-{ue.semester}-{ue.code}-{ecue_name}')},
            )
    ...
    print(f'[migration 0004] {updated_count} documents rattachés aux ECUE!')
